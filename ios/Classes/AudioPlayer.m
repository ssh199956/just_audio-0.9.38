...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    @try {
        ...
        if ([@"load" isEqualToString:call.method]) {
            ...
            result(@{});
        }  else if ([@"takeDuration" isEqualToString:call.method]) {
            long long duration = [self getDurationMicroseconds];
            result(@{@"duration": @(duration)});
        } else if ([@"takePosition" isEqualToString:call.method]) {
            long long position = [self getCurrentPosition];
...
- (void)broadcastPlaybackEvent {
    NSLog(@"hahahahahahhahahaha-getDurationMicroseconds: %i", [self getDurationMicroseconds]);
    NSLog(@"hahahahahahhahahaha-getBufferedPosition: %i", [self getBufferedPosition]);
//    NSLog(@"hahahahahahhahahaha-getBufferedPosition: %i", [self getBufferedPosition]);
...
- (long long)getDurationMicroseconds {
    ...
    NSLog(@"哈哈---》%i", duration);
    NSLog(@"哈哈--duration-》%lld", ((long long)1000 * duration));
    return duration < 0 ? -1 : ((long long)1000 * duration);
}

- (long long)getPositionMicroseconds {
    int duration = _updatePosition;
    NSLog(@"哈哈--position-》%lld", ((long long)1000 * duration));
    return duration < 0 ? -1 : ((long long)1000 * duration);
}
